...
const anim = mixer.clipAction(fbx.animations[0]);anim.setLoop(THREE.LoopRepeat);
anim.clampWhenFinished = false;anim.enabled = true;anim.play(); }
model = fbx;model.scale.setScalar(1.0);
model.rotation.set(0, Math.PI / 2, 0);originalRotation = model.rotation.clone();
const box = new THREE.Box3().setFromObject(model);
const center = box.getCenter(new THREE.Vector3());model.position.sub(center);
scene.add(model);},undefined,() => {} ); // chud
...
if (model) {model.rotation.copy(originalRotation);currentAxis = 'y';}}, 4250);}}
...
if (model) {model.rotation[currentAxis] += 0.05;}
