...
let follow = localStorage.getItem("helloT") === "true";
let hasV = localStorage.getItem("visited") === "true";
...
function toggleFollow() {
  ...
  followersRef.transaction((currentFollowers) => {
    ...
    return follow ? Math.max(0, count - 1) : count + 1;
  }, (error, committed) => {
    if (committed) {
      follow = !follow;
      localStorage.setItem("helloT", follow);
      updateButtonUI();
    } else if (error) {
