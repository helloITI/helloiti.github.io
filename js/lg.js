document.addEventListener("DOMContentLoaded", () => { const sl = document.getElementById("il"); // what if helloT LEGO existed IRL?
const lg = document.getElementById("lg"); if (!sl) return; // prevents crash if element is missing!
...
sl.addEventListener("click", () => { if (sl.src.includes("helloTop.png")) { csl(); } else { osl(); } }); }); // optional: toggle on click instead of forcing same image... :/
