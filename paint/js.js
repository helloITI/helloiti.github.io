...
function gp(e){
const rect = cv.getBoundingClientRect();const x = (e.touches ? e.touches[0].clientX : e.clientX) - rect.left;const y = (e.touches ? e.touches[0].clientY : e.clientY) - rect.top;return {x,y}; }
function st(e){
e.preventDefault();const pos = gp(e);
if(m === 'fill'){
ps(); ff(Math.floor(pos.x), Math.floor(pos.y), bc);
} else {
dr = true;lst = pos;ctx.beginPath();ctx.moveTo(pos.x, pos.y); ps(); }
}
function dw(e){ if(!dr) return;
e.preventDefault();const pos = gp(e);ctx.lineCap = 'round';ctx.lineJoin = 'round';ctx.lineWidth = bs;
if(m === 'erase'){
ctx.globalCompositeOperation = 'source-over';ctx.strokeStyle = 'white';
}else{
ctx.globalCompositeOperation = 'source-over';ctx.strokeStyle = bc; }
ctx.lineTo(pos.x, pos.y);ctx.stroke();lst = pos; }
function sp(e){
if(dr) {
e.preventDefault();dr = false;
}
}
...
m = m === 'erase' ? 'draw' : 'erase';eb.textContent = m === 'erase' ? 'Brush' : 'Eraser';
});
fb.addEventListener('click', ()=>{
m = m === 'fill' ? 'draw' : 'fill';fb.textContent = m === 'fill' ? 'Brush' : 'Fill';
});
ub.addEventListener('click', async ()=>{
if(!us.length) return;const ls = us.pop(); rs.push(cv.toDataURL()); await rdu(ls);
});
rb.addEventListener('click', async ()=>{
if(!rs.length) return;const s = rs.pop();us.push(cv.toDataURL());await rdu(s);
});
