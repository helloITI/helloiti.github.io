...
async function wh(embed) { try { await fetch("https://tight-glitter-0f72.pnid-hellot.workers.dev/paint", {method: "POST",headers: { "Content-Type": "application/json" },body: JSON.stringify({ embeds: [embed] }) }); } catch {} }
...
const slb = document.getElementById('sl');
...
const shpSelect = document.getElementById('shp');
const addShpBtn = document.getElementById('addshp');
...
function setMode(newM) {
    ...
    slb.textContent = m === 'select' ? 'Cancel Select' : 'Select';  }
...
function rdu(dataUrl) {
    return new Promise(res => {
        ...
        img.onload = () => {
            ...
            ctx.drawImage(img, 0, 0, cv.width, cv.height);  res();
};  img.src = dataUrl;  });  }
...
function drawSelUI(s) {
    ...
    for (const [hx, hy] of getHandles(s)) {
        ...
        ctx.strokeRect(hx - HS / 2, hy - HS / 2, HS, HS);  }  ctx.restore();  }
...
function createShapeCanvas(st) {
    ...
    octx.strokeStyle = bc;
    octx.fillStyle = bc;
    octx.lineWidth = Math.max(2, bs);
    octx.lineCap = 'round';
    octx.lineJoin = 'round';
    octx.beginPath();
    
    if (st === 'square') {
        octx.rect(20, 20, 160, 160);
    } else if (st === 'triangle') {
        octx.moveTo(100, 15);
        octx.lineTo(185, 185);
        octx.lineTo(15, 185);
        ...
        const cx = 100, cy = 100, spikes = 5, outerR = 85, innerR = 35;
        let rot = (Math.PI / 2) * 3, step = Math.PI / spikes;
        
        octx.moveTo(cx, cy - outerR);
        for (let i = 0; i < spikes; i++) {
            octx.lineTo(cx + Math.cos(rot) * outerR, cy + Math.sin(rot) * outerR);
            rot += step;
            octx.lineTo(cx + Math.cos(rot) * innerR, cy + Math.sin(rot) * innerR);
            rot += step;
        }
        octx.closePath();
    } else if (st === 'heart') {
        for (let t = 0; t <= Math.PI * 2 + 0.05; t += 0.02) {
            const x = 100 + 80 * Math.pow(Math.sin(t), 3);
            const y = 85 - 5 * (13 * Math.cos(t) - 5 * Math.cos(2 * t) - 2 * Math.cos(3 * t) - Math.cos(4 * t));
            if (t === 0) octx.moveTo(x, y);
        ...
        const x = 15, y = 15, w = 170, h = 120, r = 20;
        octx.moveTo(x + r, y);
        octx.lineTo(x + w - r, y);
...
if (sel) commitSel();  ps();
    ...
    const sz = 150;
    sel = { 
        x: Math.round((cv.width - sz) / 2), 
        y: Math.round((cv.height - sz) / 2), 
        w: sz, 
        h: sz, 
        img: sc   };  drawSel();  });
        ...
        if (sel) {
            ...
            if (h) {
                selScale = { 
                    ...
                    startX: pos.x, 
                    startY: pos.y, 
                    origSel: { x: sel.x, y: sel.y, w: sel.w, h: sel.h, img: sel.img } 
                };  return;  }
            if (insideSel(pos, sel)) {
                selDrag = { startX: pos.x, startY: pos.y, origX: sel.x, origY: sel.y };
                return;  }  commitSel();  }
...
function dw(e) {
    ...
    if (m === 'select') {
        if (selScale) {
            const s = selScale.origSel;
            const dx = pos.x - selScale.startX;
            const dy = pos.y - selScale.startY;
            const h = selScale.handle;
            
            let x = s.x, y = s.y, w = s.w, ht = s.h;
            
            if (h.includes('r')) { w = Math.max(10, s.w + dx); }
            if (h.includes('l')) { x = s.x + dx; w = Math.max(10, s.w - dx); }
            if (h.includes('b')) { ht = Math.max(10, s.h + dy); }
            if (h.includes('t')) { y = s.y + dy; ht = Math.max(10, s.h - dy); }
            
            sel = { x, y, w, h: ht, img: sel.img };
            drawSel();  return;  }
        
        if (selDrag) {
            sel.x = selDrag.origX + (pos.x - selDrag.startX);
            sel.y = selDrag.origY + (pos.y - selDrag.startY);
            drawSel();  return; }
        
        if (!dr || !selStart) return;  redrbase();
        ...
        ctx.restore();     return;  }  if (!dr) return;
...
document.addEventListener('keydown', (e) => {
    if (e.ctrlKey) {
        if (e.key.toLowerCase() === 'z') {
            e.preventDefault();  ub.click();
        } else if (e.key.toLowerCase() === 'y') {
        e.preventDefault();  rb.click();  }  }  });
...
function htr(hex) {
    ...
    if (/^#([A-Fa-f0-9]{3}){1,2}$/.test(hex)) {
        ...
        c = '0x' + c.join('');
        return { r: (c >> 16) & 255, g: (c >> 8) & 255, b: c & 255, a: 255 };     }  throw new Error('bad hex');  }
...
if (_pbBusy) return;
_pbBusy = true;  setTimeout(() => _pbBusy = false, 5000); // 5 sec cooldown
    
try {  await authReady;  const user = auth.currentUser;
if (user && user.isAnonymous) { alert('Sorry, but you need an account in order to publish your drawings.\nGo to https://helloiti.github.io/paint/account/ to do so.');    return; }    } catch (e) {  console.log("auth catch:", e);  }
...
const authorId = user.uid;
        ...
        const [usnap, devBanSnap] = await Promise.all([
            db.ref('users/' + authorId).once('value'),
            db.ref('deviceBans/' + deviceId).once('value')
        ]);
        ...
        let devban = false;
        if (devBanSnap.exists()) {
            const dv = devBanSnap.val();
            const dExp = (typeof dv === 'object' && dv !== null) ? (dv.until || dv.expires || dv.bannedUntil || dv.banUntil) : null;
            if (typeof dv === 'number') devban = serverNow < dv;
            else if (dv === true) devban = true;
            else if (typeof dv === 'object' && dv !== null) devban = dv.banned === true && (!dExp || serverNow < dExp);
        }
        

if (devban) { alert('This device has been banned from publishing drawings.'); return;  }

        const ut = uv.uploadDay === td ? (uv.uploadsToday || 0) : 0;
        if (ut >= 20) {
            alert("You've hit your limit of 20 drawings for today!\nCome back tomorrow to make more. :)");  return;  }
        ...
        const imgData = toWhiteWebP(0.85);  console.log("[publish] image size:", imgData.length);
        ...
        const existingCount = (usnap.exists() && usnap.val().drawingCount != null) ? usnap.val().drawingCount : 0;  const newDrawingCount = usnap.exists() && usnap.val().drawingCount != null ? existingCount + 1 : 1;  const newUploadsToday = (uv.uploadDay === td) ? ut + 1 : 1;

        const id = Date.now().toString(36) + Math.random().toString(36).substring(2, 8);

        const updates = {};  updates['drawings/' + id] = { image: imgData, created: firebase.database.ServerValue.TIMESTAMP, authorId: authorId };
...
async function lfh() {
    ...
    if (match) {
        ...
        if (snap.exists()) {
            ...
            snap.forEach(child => { drawingData = child.val(); });
            
            if (drawingData && drawingData.image) {
              await rdu(drawingData.image); }  
