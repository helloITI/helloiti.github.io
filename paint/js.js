...
let sel = null; // {x,y,w,h,img}
let selDrag = null; // {startX,startY,origX,origY}
let selScale = null; // {handle,startX,startY,origSel}
const HANDLE_SIZE = 8;
function setMode(newM) {
if (m === 'select' && newM !== 'select' && sel) commitSel();m = newM;eb.textContent = m === 'erase' ? 'Brush' : 'Eraser';fb.textContent = m === 'fill' ? 'Brush' : 'Fill';slb.textContent = m === 'select' ? 'Cancel Select' : 'Select'; }
function ps() { if(us.length >= mu) us.shift();us.push(cv.toDataURL());rs.length = 0; }
function rdu(dataUrl){
return new Promise(res=>{
const img = new Image();
img.onload = ()=>{ ctx.clearRect(0,0,cv.width,cv.height);ctx.drawImage(img,0,0,cv.width,cv.height);res(); };
img.src = dataUrl;
}); }
function gp(e){
const rect = cv.getBoundingClientRect();
const scaleX = cv.width / rect.width;const scaleY = cv.height / rect.height;
...
ctx.strokeStyle = '#00aaff';ctx.lineWidth = 1;ctx.setLineDash([5,3]);
ctx.strokeRect(sel.x, sel.y, sel.w, sel.h);
ctx.setLineDash([]);
for (const [hx,hy] of getHandles(sel)) {
ctx.fillStyle = 'white';ctx.strokeStyle = '#00aaff';ctx.lineWidth = 1;ctx.fillRect(hx - HANDLE_SIZE/2, hy - HANDLE_SIZE/2, HANDLE_SIZE, HANDLE_SIZE);ctx.strokeRect(hx - HANDLE_SIZE/2, hy - HANDLE_SIZE/2, HANDLE_SIZE, HANDLE_SIZE); }
ctx.restore(); }
...
function hitHandle(pos, s) {const handles = getHandles(s);const names = ['tl','tm','tr','ml','mr','bl','bm','br'];
for (let i = 0; i < handles.length; i++) {
const [hx,hy] = handles[i];if (Math.abs(pos.x - hx) <= HANDLE_SIZE && Math.abs(pos.y - hy) <= HANDLE_SIZE) return names[i]; }
return null;}
function insideSel(pos, s) {return pos.x >= s.x && pos.x <= s.x+s.w && pos.y >= s.y && pos.y <= s.y+s.h;}
function commitSel() {
if (!sel) return;ctx.drawImage(sel.img, sel.x, sel.y, sel.w, sel.h);sel = null;selDrag = null;selScale = null;}
function redrawBase() { const tmp = us[us.length - 1];if (tmp) { const img = new Image();img.src = tmp;ctx.clearRect(0,0,cv.width,cv.height);ctx.drawImage(img,0,0,cv.width,cv.height); } }
function st(e){
e.preventDefault();const pos = gp(e);
if (m === 'select') {
if (sel) {
const h = hitHandle(pos, sel);
if (h) { selScale = {handle:h, startX:pos.x, startY:pos.y, origSel:{...sel}}; return; }
if (insideSel(pos, sel)) { selDrag = {startX:pos.x, startY:pos.y, origX:sel.x, origY:sel.y}; return; }
commitSel(); } dr = true;lst = pos;return; }
...
let {x,y,w,h:ht} = s;if (h.includes('r')) { w = Math.max(10, s.w + dx); }
if (h.includes('l')) { x = s.x + dx; w = Math.max(10, s.w - dx); }if (h.includes('b')) { ht = Math.max(10, s.h + dy); }
if (h.includes('t')) { y = s.y + dy; ht = Math.max(10, s.h - dy); }sel = {...sel, x, y, w, h:ht};
redrawBase();drawSel();return;}
if (selDrag) {
sel.x = selScale ? sel.x : selDrag.origX + (pos.x - selDrag.startX);sel.y = selDrag.origY + (pos.y - selDrag.startY);redrawBase();drawSel();return; }
if (!dr) return;redrawBase();
const rx = Math.min(lst.x, pos.x);const ry = Math.min(lst.y, pos.y);const rw = Math.abs(pos.x - lst.x);const rh = Math.abs(pos.y - lst.y);
...
function sp(e){
if (m === 'select') {
if (selScale) { selScale = null; return; }if (selDrag) { selDrag = null; return; }if (!dr) return;
dr = false;const pos = gp(e);const rx = Math.min(lst.x, pos.x);const ry = Math.min(lst.y, pos.y);
const rw = Math.abs(pos.x - lst.x);const rh = Math.abs(pos.y - lst.y);
if (rw < 2 || rh < 2) { redrawBase(); return; }
ps();const imgData = ctx.getImageData(rx, ry, rw, rh);ctx.fillStyle = 'white';ctx.fillRect(rx, ry, rw, rh);
const oc = document.createElement('canvas');oc.width = rw;oc.height = rh;
oc.getContext('2d').putImageData(imgData, 0, 0);sel = {x:rx, y:ry, w:rw, h:rh, img:oc};drawSel();return;}
...
if (sel) { commitSel(); }
if(!us.length) return;const ls = us.pop(); rs.push(cv.toDataURL()); await rdu(ls);sel=null; });
rb.addEventListener('click', async ()=>{
if(!rs.length) return;const s = rs.pop();us.push(cv.toDataURL());await rdu(s);sel=null; });
cb.addEventListener('click', ()=>{
if (sel) commitSel();
ps();ctx.fillStyle = 'white'; ctx.fillRect(0,0,cv.width,cv.height); });
dbtn.addEventListener('click', ()=>{
if (sel) commitSel();
const a = document.createElement('a');a.href = cv.toDataURL();a.download = 'painting.png';a.click(); });
...
cob.textContent = '※ Copied Link! ※';setTimeout(()=>cob.textContent = '※ Copy Link ※',1000); } });
function htr(hex){
...
let x = sx; let y = sy;
while(x >= 0){
const pos = pk(x,y); if(!cm({r:data[pos],g:data[pos+1],b:data[pos+2],a:data[pos+3]}, sc)) break; x--; }
x++;let ru = false; let rd = false;
for(let nx = x; nx < w; nx++){
const pos = pk(nx,y); if(!cm({r:data[pos],g:data[pos+1],b:data[pos+2],a:data[pos+3]}, sc)) break; data[pos] = tc.r; data[pos+1] = tc.g; data[pos+2] = tc.b; data[pos+3] = tc.a;
if(y > 0){
const up = pk(nx, y-1);
...
} else { alert('I couldnt generate your link... error: ' + e.message); } } });
async function lfh(){
