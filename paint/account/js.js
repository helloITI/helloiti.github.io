...
// DOM Elements
var r_u = document.getElementById("r_u"); var r_e = document.getElementById("r_e");
var r_p = document.getElementById("r_p"); var r_cp = document.getElementById("r_cp");
var r_sp = document.getElementById("r_sp"); var r_msg = document.getElementById("r_msg");

var l_u = document.getElementById("l_u"); var l_e = document.getElementById("l_e");
var l_p = document.getElementById("l_p"); var l_cp = document.getElementById("l_cp");
var l_sp = document.getElementById("l_sp"); var l_msg = document.getElementById("l_msg");

var b_sup = document.getElementById("b_sup"); var b_lg = document.getElementById("b_lg");
var b_lt = document.getElementById("b_lt"); var b_gl = document.getElementById("b_gl");

var act_c = document.getElementById("act_c"); var act_i = document.getElementById("act_i");
var l_cd = document.getElementById("l_cd"); var s_cd = document.getElementById("s_cd");

var g_div = document.getElementById("g_div"); var g_u = document.getElementById("g_u");
var b_gus = document.getElementById("b_gus"); var g_msg = document.getElementById("g_msg");
var pendGUS = null;

var b_ssup = document.getElementById("b_ssup"); var b_slg = document.getElementById("b_slg");
var b_slgl = document.getElementById("b_slgl"); var st_p = document.getElementById("st_p");
var b_sto = document.getElementById("b_sto"); var b_stc = document.getElementById("b_stc");

var st_nu = document.getElementById("st_nu"); var b_stu = document.getElementById("b_stu");
var st_um = document.getElementById("st_um"); var st_ne = document.getElementById("st_ne");
var st_cp_e = document.getElementById("st_cp_e"); var b_ste = document.getElementById("b_ste");
var st_em = document.getElementById("st_em"); var st_cp = document.getElementById("st_cp");
var st_np = document.getElementById("st_np"); var st_cop = document.getElementById("st_cop");
var b_stp = document.getElementById("b_stp"); var st_pm = document.getElementById("st_pm");
var st_es = document.getElementById("st_es"); var st_ps = document.getElementById("st_ps");
...
function enforceBan(msg){
  isBanned = true;
  detachUserListener();
  st_p.style.display = "none"; act_c.style.display = "none";
  l_cd.style.display = "none"; s_cd.style.display = "none";
  b_gl.style.display = "none"; if(orb) orb.style.display = "none";
...
function listenToDeviceBans() {
  if (!deviceId) return;
  db.ref("deviceBans/" + deviceId).on("value", function(devSnap) {
...
if(b_ssup) b_ssup.onclick = function(){
  l_cd.style.display = "none"; s_cd.style.display = "flex";
  b_ssup.parentElement.style.display = "none"; b_slg.style.display = "block";
  document.querySelector("h2").textContent = "※ Register your very own Paint Account! ※";
};

if(b_slgl) b_slgl.onclick = function(){
  s_cd.style.display = "none"; l_cd.style.display = "flex";
  b_ssup.parentElement.style.display = "block"; b_slg.style.display = "none";
...
function showLoggedInUI(uname, isGoogle, banned){
  ...
  b_gl.style.display = "none"; g_div.style.display = "none";
...
function showLoggedOutUI(){
  ...
  detachUserListener();
  st_p.style.display = "none"; act_c.style.display = "none";
  l_cd.style.display = "flex"; s_cd.style.display = "none";
  if(orb) orb.style.display = "block";
  if(b_ssup) b_ssup.parentElement.style.display = "block";
  if(b_slg) b_slg.style.display = "none";
  b_gl.style.display = "block"; g_div.style.display = "none";
...
// REALTIME AUTH & USER PROFILE LISTENER
auth.onAuthStateChanged(function(user){
  detachUserListener();

  if(user && !user.isAnonymous){
    userListener = db.ref("users/" + user.uid);
    userListener.on("value", function(snap) {
      if(!snap.exists()) {
        showLoggedOutUI();
        return;
      }
...
b_sto.onclick = function(){
  act_c.style.display = "none"; st_p.style.display = "flex";
  st_um.textContent = ""; st_em.textContent = ""; st_pm.textContent = "";
  st_nu.value = ""; st_ne.value = ""; st_cp_e.value = "";
  st_cp.value = ""; st_np.value = ""; st_cop.value = "";
  ...
  document.getElementById("d_e").value = ""; document.getElementById("d_p").value = "";
...
b_stu.onclick = function(){
  ...
  if(newUN.length < 3 || newUN.length > 20 || !/^[a-z0-9_]+$/.test(newUN)){
    st_um.textContent = "Usernames must be 3-20 characters: letters, numbers, underscores only."; return;
  }
  ...
  }).catch(function(err){ st_um.textContent = "※ Error: " + err.message + " ※"; });
};
...
b_stp.onclick = function(){
  var curPA = st_cp.value; var newPA = st_np.value; var coPA = st_cop.value;
  st_pm.textContent = "";
...
b_sup.onclick = function(){
  ...
  if(username.length < 3 || username.length > 20 || !/^[a-z0-9_]+$/.test(username)){
    r_msg.textContent = "Usernames must be 3-20 characters: letters, numbers, underscores only."; return;
  }
  ...
  p.then(function(){
    ...
    if(deviceId) updates["devices/" + deviceId + "/uids/" + cred.user.uid] = true;
    return db.ref().update(updates);
  }).catch(function(err){
    if(err.code === "auth/email-already-in-use"){ r_msg.textContent = "That email is already in use by another account."; }
    else if(err.message && err.message.indexOf("PERMISSION_DENIED") !== -1){ r_msg.textContent = "That username or email is already taken."; }
    else { r_msg.textContent = "※ Error: " + err.message + " ※"; }
  });
};
...
b_gl.onclick = function(){
  ...
  l_msg.textContent = ""; r_msg.textContent = "";
  var p = Promise.resolve();
  if(auth.currentUser && auth.currentUser.isAnonymous) p = auth.signOut();
...
b_gus.onclick = function(){
  ...
  if(username.length < 3 || username.length > 20 || !/^[a-z0-9_]+$/.test(username)){
    g_msg.textContent = "Usernames must be 3-20 characters: letters, numbers, underscores only."; return;
  }
  ...
  if(deviceId) updates["devices/" + deviceId + "/uids/" + user.uid] = true;
...
// DELETE ACCOUNT
var b_da1 = document.getElementById("b_da1"); var ds1 = document.getElementById("ds1");
var ds2 = document.getElementById("ds2"); var ds3 = document.getElementById("ds3");
var b_dy1 = document.getElementById("b_dy1"); var b_dn1 = document.getElementById("b_dn1");
var b_dy2 = document.getElementById("b_dy2"); var b_dn2 = document.getElementById("b_dn2");
var b_df = document.getElementById("b_df"); var b_dn3 = document.getElementById("b_dn3");
var d_e = document.getElementById("d_e"); var d_p = document.getElementById("d_p");
var d_msg = document.getElementById("d_msg");

function resetDelSteps(){
  ds1.style.display = "none"; ds2.style.display = "none"; ds3.style.display = "none";
  if(d_e) d_e.value = ""; if(d_p) d_p.value = ""; d_msg.textContent = "";
}
...
b_df.onclick = function(){
  ...
  getCredPromise.then(function(cred){
    if(!cred){ d_msg.textContent = "※ Authentication failed. ※"; return Promise.reject("no cred"); }
    ...
    if(err.code === "auth/wrong-password" || err.code === "auth/invalid-credential"){ d_msg.textContent = "Wrong email or password."; }
    else if(err.code === "auth/popup-closed-by-user"){ d_msg.textContent = "Popup closed."; }
    else { d_msg.textContent = "※ Error: " + err.message + " ※"; }
  });  
};
