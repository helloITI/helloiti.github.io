...
const gUD = document.getElementById("gUD");const gUS = document.getElementById("gUS");const btnGUS = document.getElementById("btnGUS");const gMsg = document.getElementById("gMsg");
let pendingGoogleUser = null;
const sSUP = document.getElementById("sSUP");const sLG = document.getElementById("sLG");const sLGL = document.getElementById("sLGL");let lCid = null;let sCid = null;liUS.placeholder = "username";
function rCap() {
...
if (!grecaptcha.getResponse(sCid)) { sMsg.textContent = "Complete the reCAPTCHA first. 🫩"; return; } manualAuthInProgress = true;let cred;
try {
...
lMsg.textContent = "";
if (!username || !email) {
lMsg.textContent = "Please enter both username and email.";return; }
if (!grecaptcha.getResponse(lCid)) {
lMsg.textContent = "Complete the reCAPTCHA first. 🫩";return; }
...
if (!userSnap.exists() || userSnap.val().email !== email) {
...
manualAuthInProgress = false;grecaptcha.reset(lCid); } };
btnLG.onclick = async () => {
const username = liUS.value.trim().toLowerCase(); const email = liEM.value.trim().toLowerCase(); const password = liPA.value;
lMsg.textContent = "";
if (!grecaptcha.getResponse(lCid)) {
lMsg.textContent = "Please complete the reCAPTCHA, ok?"; return; }
manualAuthInProgress = true;
try {
const snap = await get(child(ref(db), "usernames/" + username));
if (!snap.exists()) throw new Error("No account found for that username.");
await signInWithEmailAndPassword(auth, email, password);showLoggedInUI(username); 
} catch (err) {
lMsg.textContent = "※ Error: " + err.message + " ※";
} finally {
manualAuthInProgress = false;grecaptcha.reset(lCid);
...
const user = pendingGoogleUser || auth.currentUser;if (!user) { gMsg.textContent = "※ Something went wrong, please try signing in again. ※"; return; }
...
manualAuthInProgress = false; showLoggedInUI(username); } catch (err) {
...
console.error("[auth] users/<uid> lookup threw:", err);
await new Promise(r => setTimeout(r, 800));
try { 
snap = await get(child(ref(db), "users/" + user.uid));
} catch (err2) {
console.error("[auth] users/<uid> retry also failed:", err2); showLoggedInUI(user.email ? user.email.split("@")[0] : "Anonymous");return; } }
...
console.warn("[auth] users/" + user.uid + " missing in DB, recreating with username:", fallbackName);
