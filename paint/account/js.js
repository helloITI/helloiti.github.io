...
if (snap.exists() && snap.val().username) { manualAuthInProgress = false;showLoggedInUI(snap.val().username, true);
} else {
...
manualAuthInProgress = false; showLoggedInUI(username, true);
} catch (err) {
...
console.error("[auth] users/<uid> lookup threw:", err);await new Promise(r => setTimeout(r, 800));
try { snap = await get(child(ref(db), "users/" + user.uid));
} catch (err2) { console.error("[auth] users/<uid> retry also failed:", err2);showLoggedInUI(user.email ? user.email.split("@")[0] : "Anonymous", false);return; } }
