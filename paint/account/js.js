...
if (!grecaptcha.getResponse(sCid)) { sMsg.textContent = "Complete the reCAPTCHA first. 🫩"; return; }
...
if (!snap.exists()) throw new Error("No account found for that username.");const uid = snap.val().uid;
const userSnap = await get(child(ref(db), "users/" + uid));if (!userSnap.exists()) throw new Error("User data missing.");
const realEmail = userSnap.val().email;await signInWithEmailAndPassword(auth, realEmail, password);
showLoggedInUI(username);
